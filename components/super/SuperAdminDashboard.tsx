  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50">
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Tab Navigation - Card-based Shortcuts */}
        <nav className="flex flex-wrap gap-3">
          {[
            { id: 'dashboard' as SuperAdminTab, label: 'Dashboard', icon: '📊', color: 'blue' },
            { id: 'tenants' as SuperAdminTab, label: 'Tenants', icon: '👥', color: 'indigo' },
            { id: 'products' as SuperAdminTab, label: 'Products', icon: '📦', color: 'purple' },
            { id: 'onboarding' as SuperAdminTab, label: 'Onboarding', icon: '✨', color: 'green' },
            ...
            const colorClasses = {
              ...
              indigo: activeTab === tab.id ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50',
              purple: activeTab === tab.id ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-purple-600 border-purple-200 hover:bg-purple-50',
              green: activeTab === tab.id ? 'bg-green-600 text-white border-green-600' : 'bg-white text-green-600 border-green-200 hover:bg-green-50',
              orange: activeTab === tab.id ? 'bg-orange-600 text-white border-orange-600' : 'bg-white text-orange-600 border-orange-200 hover:bg-orange-50',
              pink: activeTab === tab.id ? 'bg-pink-600 text-white border-pink-600' : 'bg-white text-pink-600 border-pink-200 hover:bg-pink-50',
              teal: activeTab === tab.id ? 'bg-teal-600 text-white border-teal-600' : 'bg-white text-teal-600 border-teal-200 hover:bg-teal-50',
            };
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 rounded-xl border-2 px-5 py-2.5 text-sm font-semibold transition-all shadow-sm hover:shadow-md ${
        ...
        {activeTab === 'tenants' && (
          <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">Manage tenants and view their product subscriptions</p>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <span>Total Tenants</span>
                <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">{tenants.length}</span>
              </div>
            </div>
          ...
          {selectedTenant && editForm ? (
            <div className="space-y-6">
              {/* Tenant Products Overview - New Card View */}
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <TenantProductsView
                  ...
                  tenant={{
                    ...
                    monthlyFee: selectedTenant.subscriptionMonthlyFee ?? 0,
                    contactEmail: selectedTenant.contactEmail ?? null,
                    contactPhone: selectedTenant.contactPhone ?? null,
