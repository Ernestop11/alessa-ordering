...
export function useAlfredWebSocket() {
  ...
  useEffect(() => {
    // Try to connect, but don't fail if WebSocket unavailable
    try {
      // WebSocket URL - will fail gracefully if not available
      ...
      console.warn('[Alfred WebSocket] Not available:', error);
      setConnected(false);
    }
  }, []);
